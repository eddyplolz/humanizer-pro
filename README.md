...
Current release: **v4.11.2**, a reporting and cleanup pass on the detector-bypass work: the bypass count is now labeled `invisible` rather than `zero_width`, since it also covers tag characters and noncharacters, and the invisible math operators and Mongolian vowel separator gained tests. Just before it, v4.11.1 closed a hole where two side-by-side variation selectors shielded each other, and v4.11.0 hardened the pass against Unicode tag characters and noncharacters while keeping legitimate emoji and Indic or Arabic joiners intact. The full history back to 1.0.0 is in [CHANGELOG.md](CHANGELOG.md).
